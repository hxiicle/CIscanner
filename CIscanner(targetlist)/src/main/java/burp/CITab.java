...
public class CITab extends AbstractTableModel implements ITab, IMessageEditorController {
    ...
    // TargetList 的文本输入框
    private JTextArea targetListArea;
    ...
    public CITab(IBurpExtenderCallbacks callbacks, String name) {
        ...
        // 创建文本
        JLabel whitelistLabel = new JLabel("Targetlist :");
        // 创建文本输入框
        targetListArea = new JTextArea(5, 5);
        targetListArea.setLineWrap(true);
        // 添加滚动条
        JScrollPane whiteListScrollPane = new JScrollPane(targetListArea);
        ...
        // 白名单提示
        targetListArea.addFocusListener(new JTextAreaHintListener(targetListArea,
                "1. 目标为某个域名: www.domain1.com\n" +
                "2. 目标为某个域名的全部子域名: *.domain2.com\n" +
                "3. 目标为某个域名的部分子域名: a.*.domain2.com 或者 *.a.*.domain2.com\n" +
                "4. 目标为某个IP: 127.0.0.1\n" +
                "5. 多个目标中间加回车！"));
    ...
    private void getConfigFileContent() {
        ...
        String fileTargetList = fileGetValue(getConfigFile(),"Targetlist").replace("; ", "\n").trim();
        ...
        // Targetlist 的文本输入框
        this.targetListArea.setText(fileTargetList);
    ...
    public String getTargetList() {
        return this.targetListArea.getText().trim();
    }
    ...
    public void saveConfiguration() {
        ...
        String targetList = getTargetList().replace("\n","; ");
    ...
    public String checkAllFill() {
        ...
        if (!useCeye && !useOther) {
            ...
            if (useCeye && ceyeDnsServer.isEmpty() && ceyeToken.isEmpty() && payload.isEmpty()) {
                Content = "Fail!\nYou need to fill in <Ceye Dns Server> , <Ceye Token> , <Payload> and <Targetlist>";
            } else if (useCeye && ceyeDnsServer.isEmpty() && !ceyeToken.isEmpty() && payload.isEmpty()) {
                Content = "Fail!\nYou need to fill in <Ceye Dns Server> , <Payload> and <Targetlist>";
            } else if (useCeye && ceyeDnsServer.isEmpty() && ceyeToken.isEmpty() && !payload.isEmpty()) {
                Content = "Fail!\nYou need to fill in <Ceye Dns Server> , <Ceye Token> and <Targetlist>";
            } else if (useCeye && ceyeDnsServer.isEmpty() && !ceyeToken.isEmpty() && !payload.isEmpty()) {
                Content = "Fail!\nYou need to fill in <Ceye Dns Server> and <Targetlist>";

            } else if (useCeye && !ceyeDnsServer.isEmpty() && ceyeToken.isEmpty() && payload.isEmpty()) {
                Content = "Fail!\nYou need to fill in <Ceye Token> , <Payload> and <Targetlist>";
            } else if (useCeye && !ceyeDnsServer.isEmpty() && !ceyeToken.isEmpty() && payload.isEmpty()) {
                Content = "Fail!\nYou need to fill in <Payload> and <Targetlist>";
            } else if (useCeye && !ceyeDnsServer.isEmpty() && ceyeToken.isEmpty() && !payload.isEmpty()) {
                Content = "Fail!\nYou need to fill in <Ceye Token> and <Targetlist>";
            } else if (useCeye && !ceyeDnsServer.isEmpty() && !ceyeToken.isEmpty() && !payload.isEmpty()) {
                Content = "Fail!\nYou need to fill in <Targetlist>!";

            } else if (useOther && otherDnsServer.isEmpty() && otherToken.isEmpty() && payload.isEmpty()) {
                Content = "Fail!\nYou need to fill to use <Dns Server> , <URL With Token> , <Payload> and <Targetlist>";
            } else if (useOther && otherDnsServer.isEmpty() && !otherToken.isEmpty() && payload.isEmpty()) {
                Content = "Fail!\nYou need to fill to use <Dns Server> , <Payload> and <Targetlist>";
            } else if (useOther && otherDnsServer.isEmpty() && otherToken.isEmpty() && !payload.isEmpty()) {
                Content = "Fail!\nYou need to fill to use <Dns Server> , <URL With Token> and <Targetlist>";
            } else if (useOther && otherDnsServer.isEmpty() && !otherToken.isEmpty() && !payload.isEmpty()) {
                Content = "Fail!\nYou need to fill to use <Dns Server> and <Targetlist>";

            } else if (useOther && !otherDnsServer.isEmpty() && otherToken.isEmpty() && payload.isEmpty()) {
                Content = "Fail!\nYou need to fill to use <URL With Token> , <Payload> and <Targetlist>";
            } else if (useOther && !otherDnsServer.isEmpty() && !otherToken.isEmpty() && payload.isEmpty()) {
                Content = "Fail!\nYou need to fill to use <Payload> and <Targetlist>";
            } else if (useOther && !otherDnsServer.isEmpty() && otherToken.isEmpty() && !payload.isEmpty()) {
                Content = "Fail!\nYou need to fill to use <URL With Token> and <Targetlist>";
            } else if (useOther && !otherDnsServer.isEmpty() && !otherToken.isEmpty() && !payload.isEmpty()) {
                Content = "Fail!\nYou need to fill in <Targetlist>!";
            }
        } else if (!targetList.isEmpty()) {
    ...
    public String checkDnsFill() {
        ...
        if (useCeye == Boolean.TRUE) {
            if (!ceyeDnsServer.isEmpty() && !ceyeToken.isEmpty() && !payloadString.isEmpty() && !targetList.isEmpty()) {
                return "Success!";
            }
        }
        if (useOther == Boolean.TRUE) {
            if (!otherDnsServer.isEmpty() && !otherToken.isEmpty() && !payloadString.isEmpty() && !targetList.isEmpty()) {
                return "Success!";
            }
        }
        return "Fail!";
